...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addItem("➕ Cadastrar Produto — 16 Fusos", "abrirCadastroPedido16")
    .addSeparator()
...
var PREFIXO_DADOS_16 = "DADOS_16_";
...
// ═══════════════════════════════════════════════════════════
// CADASTRO DE PRODUTOS — 16 FUSOS
// ═══════════════════════════════════════════════════════════

/**
 * Abre o formulário de cadastro de produto para máquinas de 16 fusos.
 */
function abrirCadastroPedido16() {
  var html = HtmlService.createHtmlOutputFromFile("CadastroPedido16")
    .setWidth(520)
    .setHeight(620)
    .setTitle("Cadastrar Produto — 16 Fusos");
  SpreadsheetApp.getUi().showModalDialog(html, "Inserir Dados do Produto — 16 Fusos");
}

/**
 * Retorna ao formulário HTML os dados das abas DADOS_16_* para autocomplete e tabela de máquinas.
 */
function carregarDadosCadastro16() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var abas = ss.getSheets();
  ...
  for (var i = 0; i < abas.length; i++) {
    var aba  = abas[i];
    var nome = aba.getName().trim();
    if (nome.indexOf(PREFIXO_DADOS_16) !== 0) continue;
...
/**
 * Retorna todos os dados vinculados a uma referência nas abas DADOS_16_*.
 */
function buscarDadosPorReferencia16(ref) {
  var ss   = SpreadsheetApp.getActiveSpreadsheet();
  var abas = ss.getSheets();
  var refN = ref.trim().toUpperCase();
...
/**
 * Insere o cadastro nas abas DADOS_16_* selecionadas.
 * payload = { referencia, cor, largura, maquinas: [{nomeAba, tempo}] }
 */
function inserirCadastroPedido16(payload) {
  var ss  = SpreadsheetApp.getActiveSpreadsheet();
  var ref  = String(payload.referencia || "").trim();
  var cor  = String(payload.cor  || "").trim();
  var larg = String(payload.largura || "").trim();
  ...
  for (var i = 0; i < maquinasSel.length; i++) {
    ...
    if (nomeAba.indexOf(PREFIXO_DADOS_16) !== 0) {
      erros.push(nomeAba + ": aba não pertence ao grupo 16 fusos");
      continue;
    }
